...
<form id="form-cadastro" class="card p-3 mb-3">
  <input type="text" id="input-nome" class="form-control mb-2" placeholder="Nome">
  <input type="email" id="input-email" class="form-control mb-2" placeholder="E-mail">
  <button type="submit" id="btn-salvar" class="btn btn-primary">Salvar Cliente</button>
</form>

<!-- Caixa de Alerta para Feedback Visual (#caixa-alerta) -->
<div id="caixa-alerta" class="alert alert-info d-none"></div>
...
Abaixo está a progressão funcional realizada sobre a mesma estrutura:

\begin{itemize}
\item \textbf{Exemplo 6 (Passo 1 — Leitura \& Escrita de Campos):} Captura a submissão do \texttt{\#form-cadastro}, impede o recarregamento com \texttt{e.preventDefault()}, lê os valores de \texttt{\#input-nome} e \texttt{\#input-email} com \texttt{.val()} e escreve na caixa \texttt{\#caixa-alerta} com \texttt{.html()};

\item \textbf{Exemplo 7 (Passo 2 — Animações Visuais \& Classes CSS):} Incrementa a mesma \texttt{\#caixa-alerta} removendo a classe \texttt{.d-none}, aplicando \texttt{.addClass('alert-success')} e fazendo o alerta surgir suavemente com \texttt{.fadeIn(400)};

\item \textbf{Exemplo 8 (Passo 3 — Manipulação Dinâmica do DOM \& Exclusão):} Insere a nova linha \texttt{<tr>} com os dados e o botão \texttt{.btn-excluir} diretamente na \texttt{\#tabela-corpo} com \texttt{.append()}. Ao clicar em Excluir, remove a linha pai da tabela com \texttt{\$(this).closest('tr').remove()};

\item \textbf{Exemplo 9 (Passo 4 — Persistência no LocalStorage com JSON):} Salva o array de clientes no navegador com \texttt{localStorage.setItem('lista\_clientes', JSON.stringify(clientes))} e restaura a \texttt{\#tabela-corpo} ao pressionar F5 com \texttt{JSON.parse()}.
\end{itemize}
